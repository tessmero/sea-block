...
const handlers: ReadonlyArray<EventHandler> = [
  ...
  },
  {
    ...
    action: (event, context) => {
      ...
      if (!isDraggingOrbitControls && !isZoomingOrbitControls) {
        consumed = context.game.gui.move(event)
      }

      if (!isDraggingOrbitControls && !consumed && dragOrbitId
      ...
      if (isDraggingOrbitControls && !consumed) {
  ...
  },
  {
    ...
    action: (event, context) => {
      ...
      const wasConsumed = context.game.gui.click(event)

      if (!wasConsumed && (typeof dragOrbitId === 'undefined')) {
        dragOrbitId = event
        const orbitControls = context.orbitControls as unknown as HackedOrbitControls
